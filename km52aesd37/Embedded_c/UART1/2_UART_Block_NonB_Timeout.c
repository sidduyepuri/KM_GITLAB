...
unsigned char KM_UART1_Blocking(void)
{
...
unsigned char KM_UART1_Non_Block(void)
{
	if(UART1_SR&0x1<<5)//check Receiver full
	{
		return (UART1_DR&0xFF);//return data
	}
}

unsigned char KM_UART1_Timeout(void)
{
	int x=100;
	while(x--)
	{
...
//		ch = KM_UART1_Blocking();//Blocking
//		ch = KM_UART1_Non_Block();//Non_Blocking
		ch = KM_UART1_Timeout();//Timeout 
		ch++;
	}
}
